...
def callback_cam12(pc2_msg1, pc2_msg2):
    o3dpc1 = orh.rospc_to_o3dpc(pc2_msg1)
    o3dpc2 = orh.rospc_to_o3dpc(pc2_msg2)
    np_pc1 = np.array(o3dpc1.points)
    np_pc2 = np.array(o3dpc2.points)
    np_pc = np.concatenate((np_pc1, np_pc2), axis=0)
...
if __name__ == '__main__':
    ...
    rospy.init_node('merge', disable_signals=True)
    cam1_sub = message_filters.Subscriber('/' + base_cam, pc2.PointCloud2)
    cam2_sub = message_filters.Subscriber('/transformed_' + side_cam, pc2.PointCloud2)
    ts = message_filters.ApproximateTimeSynchronizer([cam1_sub, cam2_sub], 1, 1)
    ts.registerCallback(callback_cam12)
    pub = rospy.Publisher('/merged', pc2.PointCloud2, queue_size=10)
    rospy.spin()
