...
def track(matrix_coefficients, distortion_coefficients):
    ...
    while True:
        ...
        frame = np.asanyarray(color_frm.get_data())
        ...
        if np.all(ids is not None):  # If there are markers found by detector
            ...
            for i in range(0, len(ids)):  # Iterate in markers
                ...
                rvec, tvec, markerPoints = aruco.estimatePoseSingleMarkers(corners[i], 0.02, camera_matrix, dist)
            ...
            if len(ids) > 1 and composedRvec is not None and composedTvec is not None:
                ...
                imgpts, jac = cv2.projectPoints(axis, TcomposedRvec, TcomposedTvec, camera_matrix,
                                                dist)
...
if __name__ == '__main__':
    ...
    s = cv2.FileStorage()
    cam_calibration_file = '../config/cam_calib_0.xml'
